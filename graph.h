...
public:
    ...
    void run_dijkstra(const T& start_vertex, QTextEdit* output) {
        ...
        output->append("Shortest paths from " + QString::fromStdString(start_vertex) + " to all other cities:\n");
        ...
        for (int i = 0; i < cityCount; ++i) {
            ...
            if (path.size() <= 1) continue;  // No actual path

            // Build path string (skip start_vertex)
            QString pathStr;
            ...
            double pathDistance = distance[i];
            totalPathDistance += pathDistance;

            output->append(pathStr + " (Distance: " + QString::number(pathDistance) + ")");
        }
...
//     output->append("Shortest paths from " + QString::fromStdString(start_vertex) + ":\n");
//     double totalPathDistance = 0;
